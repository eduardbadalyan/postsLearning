    ...
    $result = $mysqli->query ("SELECT * FROM `posts` ORDER BY posts.id");
        ...
        if($_SESSION["name"] != ""){
            if(is_null($like_result) == true) {
                $mysqli->query ("INSERT INTO `likes` VALUES (NULL,'".$_POST["post_id"]."','".$_SESSION["user_id"]."',0);");
                $like = 0;
                $dislike = 1;
                echo $like.",".$dislike;  
            }else{
            if($like_result == 1){
                $mysqli->query ("UPDATE `likes` SET `result` = '0' WHERE `likes`.`post_id` = ".$_POST["post_id"]." AND `likes`.`user_id` = ".$_SESSION["user_id"].";");
                $like = -1;
                $dislike = 1;
                echo $like.",".$dislike;  
            }
